...
volatile uint8_t *Driver_PcmBuf = NULL;
...
bool Driver_Setup() {
	...
	ESP_LOGI(TAG, "Heap size=%08X",esp_get_free_heap_size());
	
	Driver_PcmBuf =  malloc(DACSTREAM_BUF_SIZE*DACSTREAM_PRE_COUNT);
	if (Driver_PcmBuf == NULL) {
        ESP_LOGI(TAG, "Couldn't malloc Driver_PcmBuf object !!");
        return 0xff;
    }
	...
	ESP_LOGI(TAG, "Driver_PcmBuf Heap size=%08X",esp_get_free_heap_size());
...
void Driver_ModDetect() {
	...
	Driver_SrBuf[SR_CONTROL] |= SR_BIT_A0; //HIGH A0
...
void Driver_Main() {
	...
	if(Driver_DetectedMod == MEGAMOD_YMU76X) Driver_Ymu762Init();
	ESP_LOGI(TAG, "Driver_PcmBuf Heap size=%08X",esp_get_free_heap_size());
    while (1) {
